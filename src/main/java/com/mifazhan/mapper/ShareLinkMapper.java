...
public interface ShareLinkMapper extends BaseMapper<ShareLink> {
    ...
    @Update("<script>" +
            "UPDATE share_link SET deleted = 0 WHERE target_type = #{targetType} AND " +
            "<if test='targetType == 2'>project_id = #{targetId}</if>" +
            "<if test='targetType != 2'>node_id = #{targetId}</if>" +
            "</script>")
    void restoreShareByTarget(@Param("targetType") Integer targetType, @Param("targetId") Long targetId);

    @Update("<script>" +
            "UPDATE share_link SET deleted = 0 WHERE target_type IN (0, 1) AND node_id IN " +
            "<foreach collection='nodeIds' item='id' open='(' separator=',' close=')'>#{id}</foreach>" +
            "</script>")
    void restoreNodeShares(@Param("nodeIds") List<Long> nodeIds);

    @Delete("<script>" +
            "DELETE FROM share_link WHERE target_type = #{targetType} AND " +
            "<if test='targetType == 2'>project_id = #{targetId}</if>" +
            "<if test='targetType != 2'>node_id = #{targetId}</if>" +
            "</script>")
    ...
    @Delete("<script>" +
            "DELETE FROM share_link WHERE target_type IN (0, 1) AND node_id IN " +
            "<foreach collection='nodeIds' item='id' open='(' separator=',' close=')'>#{id}</foreach>" +
            "</script>")
